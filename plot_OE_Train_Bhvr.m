function [fHandle] = plot_OE_Train_Bhvr(cohortStruc)
% 3/2/2021 LKW
%Inputs: 
%cohortStruc = struct of one field (table) for each mouse in cohort
%Use getSessBhvr.m to set up table for this function
...
for i = 1:nMice
    ...
    if height(tTmp) < longestTrain
        ...
        accMat(i,:) = [tTmp.accuracy' nan(1,sessDiff)];
        trialsMat(i,:) = [tTmp.nTrials' nan(1,sessDiff)];
    else
        accMat(i,:) = tTmp.accuracy;
...
legend('Group Average','Individual Mice','location','southeast');
end
